...
def bcast_addr_gen(address, mask):
        bcast_parts=[]
        ipaddr_parts=address.split('.')
        mask_parts=mask.split('.')
        for i in [0,1,2,3]:
                if mask_parts[i]=='255':
                        bcast_parts.append(ipaddr_parts[i])
                else:
                        bcast_parts.append('255')
        bcastaddr=bcast_parts[0]+'.'+bcast_parts[1]+'.'+bcast_parts[2]+'.'+bcast_parts[3]
        return bcastaddr
...
def bcast_addr_all():
	myifaces=interfaces()
	bcastaddr_array=[]
	for ifaceName in myifaces:
        	addr= ifaddresses(ifaceName).setdefault(AF_INET, [{'addr':'No IP addr'}])
        	addresses=addr[0]['addr']
        	if not addresses=='No IP addr':
                	mask = addr[0]['netmask']
                	bcastaddr=bcast_addr_gen(addresses, mask)
                	bcastaddr_array.append(bcastaddr)
                	print '%s: %s %s %s' % (ifaceName, addresses, mask, bcastaddr)
	return bcastaddr_array
...
def bcast_prober():
    ...
    while True:
	if opts.bcast_enable==False: #bcast_enable default true
		time.sleep(4)
		continue	
	bcast_array= bcast_addr_all()
 	for i in bcast_array:
        	s.sendto('this is testing',(i,14550))
		...
		try: 
        		m=s.recvfrom(1024)
        		print m
#port format udp:0.0.0.0:14550
			newport='udp:'+m[1][0]+":"+str(m[1][1])
			print newport
        		mpstate.mav_outputs.append(mavutil.mavlink_connection(newport, baud=int(opts.baudrate), input=False))
	
			print("bcast prober")
        		time.sleep(1)
    		except Exception as e:
        		print("bcast recv timeout %s" % e)
			time.sleep(1)
        	#return
...
if __name__ == '__main__':
    ...
    parser.add_option("--bcast_enable", dest="bcast_enable", type='int',
                      help="default bcast enable?", default=1)
    ...
    #create bcast probing thread
    print 'start bcast thread'
	
    t_bcast = threading.Thread(target=bcast_prober, name='bcast_prober')
    t_bcast.daemon = True
    t_bcast.start()
